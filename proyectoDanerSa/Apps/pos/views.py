...
def ListaVentasgraph(request):
    ...
    if request.method == 'POST':
        ...
        if opcion == 'dia':
            ...
            if not fecha_dia:
                return render(request, 'pos/grafica.html', {'error': 'Por favor, seleccione una fecha para la opción "Día".'})

            # Validar el formato de la fecha
            try:
                datetime.strptime(fecha_dia, '%Y-%m-%d')
            except ValueError:
                return render(request, 'pos/grafica.html', {'error': 'Formato de fecha no válido. Utilice YYYY-MM-DD.'})
            ...
            if not fecha_inicio or not fecha_fin:
                return render(request, 'pos/grafica.html', {'error': 'Por favor, complete ambas fechas para la opción "Rango de Fecha".'})
            ...
            # Si la fecha está en blanco, mostrar un mensaje de error
            if not fecha_mes:
                return render(request, 'pos/grafica.html', {'error': 'Por favor, seleccione un mes y año para la opción "Mes".'})

            # Validar el formato del mes y año
            try:
                datetime.strptime(fecha_mes, '%Y-%m')
            except ValueError:
                return render(request, 'pos/grafica.html', {'error': 'Formato de fecha no válido. Utilice YYYY-MM.'})
        ...
        return render(request, 'pos/grafica.html', {'chart_data': chart_data, 'opcion': opcion, 'fecha_dia': fecha_dia, 'fecha_inicio': fecha_inicio, 'fecha_fin': fecha_fin, 'fecha_mes': fecha_mes, 'error': None})

    # Esta parte se ejecutará si la solicitud es un GET
    return render(request, 'pos/grafica.html', {'chart_data': None, 'opcion': None, 'fecha_dia': None, 'fecha_inicio': None, 'fecha_fin': None, 'fecha_mes': None, 'error': None})
